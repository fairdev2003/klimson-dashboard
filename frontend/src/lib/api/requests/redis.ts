...
export class Redis {
	...
	public async GetKeys(): Promise<ServerResponse<BackendResponse<{ rdbs: string[] }>>> {
		const response: ServerResponse<BackendResponse<{ rdbs: string[] }>> = await this.api.get(
			`/admin/redis/keys`,
			{
	...
	public async PingRedis(): Promise<ServerResponse<BackendResponse<{ ping: string }>>> {
		const response: ServerResponse<BackendResponse<{ ping: string }>> = await this.api.get(
			`/redis/ping`,
			{
				headers: { Authorization: `Bearer ${Api.token}` }
			}
