...
<svelte:document
	onkeydown={async (e) => {
		...
		if (e.key === 'Enter') {
			...
			const splitObject = commandLineValue.split(' ');
			const firstArg = splitObject[0];

			switch (firstArg) {
				case 'log': {
					if (splitObject.length < 2) {
						debug.warn('You need <object> arg to execute this command');
						return;
					}
					const secondArg = splitObject[1];
					if (secondArg === 'routes') {
						$routes?.forEach((r) => {
							debug.log(r);
						});
					...
					if (secondArg === 'ctx') {
						...
						debug.system('\n');
						const ctxs = response.data;
						debug.system('Contexts:');
