...
<Modal className="w-100" title="Zmień konto" bind:opened={changeAccountModalOpen} onClose={() => changeAccountModalOpen = false}>
            ...
            {#each $accounts as account}
                ...
                >
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold shadow-md group-hover:scale-105 transition-transform">
                    ...
                    <div class="flex items-center gap-2">
                        {#if $userInfo.login === account.login}
                            <div class="flex items-center gap-1.5 bg-blue-500/10 text-blue-400 px-3 py-1 rounded-full border border-blue-500/20">
                                <span class="w-2 h-2 bg-blue-400 rounded-full animate-pulse shadow-[0_0_8px_rgba(96,165,250,0.6)]"></span>
                                <span class="text-[10px] font-bold uppercase tracking-widest">Active</span>
                            </div>
                        {:else}
                            <span class="text-neutral-600 group-hover:text-neutral-300 group-hover:translate-x-1 transition-all">
