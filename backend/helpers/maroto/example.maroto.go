...
func Run() {
	...
	pdfPath := "static/docs/assets/pdf/billingv2.pdf"
	txtPath := "static/docs/assets/text/billingv2.txt"

	if err := os.MkdirAll(filepath.Dir(pdfPath), 0755); err != nil {
		log.Fatalf("Nie udało się utworzyć katalogu na PDF: %v", err)
	}

	if err := os.MkdirAll(filepath.Dir(txtPath), 0755); err != nil {
		log.Fatalf("Nie udało się utworzyć katalogu na TXT: %v", err)
	}
...
func getPageHeader() core.Row {
	return row.New(20).Add(
		image.NewFromFileCol(3, "static/docs/assets/images/golang.png", props.Rect{
			Center:  true,
			Percent: 80,
		}),
		col.New(6),
